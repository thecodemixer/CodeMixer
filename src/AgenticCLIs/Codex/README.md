...
- Model picker options are discovered from `~/.codex/models_cache.json` (same
  catalog as the Codex TUI) and cached daily in
  `<workspace>/.codemixer/workspace-codex.json`. Each `AgentModelOption` carries
  `code`, `name`, default `thinkingEffort`, and `supportedThinkingEfforts`.
  Selected model + effort are sent on `turn/start` as `model` and `effort`.
