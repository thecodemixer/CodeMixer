...
struct FolderView: View {
    ...
    @ViewBuilder
    private func content(_ browser: FolderViewModel) -> some View {
        VStack(spacing: 0) {
            header(browser)
            Divider()
            searchBar(browser)
    ...
    private func syncPinnedPaths(into browser: FolderViewModel) {
        browser.pinnedRelativePaths = Set(model.folderPinnedPathsByProject[project.path] ?? [])
    }

    private func header(_ browser: FolderViewModel) -> some View {
    ...
    private func searchBar(_ browser: FolderViewModel) -> some View {
        SearchFieldBar(
    ...
    private func filterBar(_ browser: FolderViewModel) -> some View {
    ...
    private func openSelected(_ browser: FolderViewModel) {
    ...
    private func copySelectedPaths(_ browser: FolderViewModel) {
        let paths = browser.selectedPaths.isEmpty
            ? Set([browser.selectedRelativePath].compactMap { $0 })
            : browser.selectedPaths
    ...
    private func quickLookSelected(_ browser: FolderViewModel) {
