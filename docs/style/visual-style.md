...
- **Not loaded:** when a shipping adapter's model catalog cannot be populated for this workspace, projects that require that adapter (pinned type, or mixed whose *default* agent is that adapter) move under a trailing **Not loaded** group. Section header uses `typography.caption` + `text.tertiary` + uppercase; each row shows the project name in `text.tertiary` and the error in `signal.warning` caption beneath — no expand/sessions, no New Chat. Context menu keeps Reveal / Rename / Remove plus **Retry model load**. Focus-mode icon rail shows these as a disabled `folder.badge.questionmark` tinted `signal.warning`.
...
- **SystemBubble**: centered, no background, `caption` text in `text.tertiary` — used for "Session resumed", "Permission granted", etc. Codemixer `ClientAction` rows (`ClientActionRowView`) follow this pattern for mode / model / slash / permission / session markers and replay from the project-local transcript.
