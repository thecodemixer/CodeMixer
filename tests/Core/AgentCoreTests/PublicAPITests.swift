...
struct ProjectTypeTests {
    ...
    func folderTypes() {
        ...
        // Every kind previews the selected file; only the chrome around it differs.
        for kind in FolderProjectKind.allCases {
            #expect(kind.showsPreviewOnSelection)
        }
        ...
        #expect(!FolderProjectKind.dualFolderTree.usesTreeNavigation)
        #expect(!FolderProjectKind.dualFolderTree.supportsPinnedSidebarEntries)
        #expect(!FolderProjectKind.dualFolderTree.usesMarkdownPreview)
        #expect(!FolderProjectKind.dualFolderTree.showsAutomaticSidebarShortcuts)
        #expect(FolderProjectKind.dualFolderTree.systemImage == "rectangle.split.2x1")
